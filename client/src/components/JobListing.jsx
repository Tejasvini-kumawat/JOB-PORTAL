  ...
  <h3 className='font-medium text-lg mb-4 '>
    ...
    <div className='mb-4 text-gray-600'>
      {searchFilter.title && (
        ...
        <img onClick={e => setSearchFilter(prev=>({...prev,title :" "} ))} className= "cursor-pointer" src={assets.cross_icon} alt="" /></span> 
      )}
      {searchFilter.location && (
        <span className=' ml-2 inline-flex items-center gap-2.5 bg-red-50 border border-red-200 px-4 py-1.5 rounded'>{searchFilter.location}
         <img onClick={ e => setSearchFilter(prev=>({...prev,location :" "} ))} className= "cursor-pointer" src={assets.cross_icon} alt="" /></span>
...
<button onClick={e =>setShowFilter(prev => !prev)} className='px-6 py-1.5 rounded border border-gray-400 max-lg:hidden'>
...
checked={selectedLocations.includes(location)} />
 {location}
      ...
      <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4'>
        ...
        <p className='mb-8'>Get your dream Job From Desired Companies</p>
...
<JobCard key={index} job={job}/>
        ...
        {/* Pagination */}
        {filterJobs.length >0 && (<div className='flex items-center justify-center space-x-2 mt-10'>
          <a href="#job-list" >
